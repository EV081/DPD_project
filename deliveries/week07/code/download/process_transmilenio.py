...
DATA = Path(__file__).resolve().parents[2] / "data" / "transmilenio"
...
def main() -> None:
    ...
    if not (args.desde and args.hasta):
        fechas = sorted(f for f in (fecha_de_nombre(p) for p in zips) if f)
        if fechas:
            def _iso(f8: str) -> str:
                return f"{f8[:4]}-{f8[4:6]}-{f8[6:8]}"
            args.desde = args.desde or _iso(fechas[0])
            args.hasta = args.hasta or _iso(fechas[-1])
    ...
    con_gtfs = bool(args.gtfs and args.gtfs.exists())
    oferta = {}
    if con_gtfs:
